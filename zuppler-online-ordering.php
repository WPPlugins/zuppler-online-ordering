...
class Zuppler_integration {
  ...
  var $scripthost = "//api.zuppler.com";
  var $channel_slug;
  ...
  function enqueue_scripts() {
    ...
    if($this->load_menu_assets) {
      wp_register_script(
        ...
        "{$this->scripthost}/channels/{$this->channel_slug}/restaurants/{$restaurant}/menu.js",
        false,
        "1.0",
        true);
    ...
    if($this->load_reviews_assets) {
      wp_register_script(
        ...
        "{$this->scripthost}/channels/{$this->channel_slug}/restaurants/{$restaurant}/reviews.js",
        false,
        "1.0",
        true);
    ...
    if($this->load_profile_assets) {
      wp_register_script(
        ...
        "{$this->scripthost}/channels/{$this->channel_slug}/restaurants/{$restaurant}/profile.js",
        false,
        "1.0",
        true);
  ...
  function show_restaurant_listing() {
    ...
    if(strpos($this->listing_template, 'is_restaurant_open') !== false) {
      ...
      foreach ($restaurants as $r) {
        $this->script_is_restaurant_open["restaurant_" . $r["id"]] = $r["hours_of_operation"]["machine"];
      }
  ...
  function get_channel_details(){
    ...
    $details = $this->get_resource($data_url, $this->channel_slug . '_details');
  ...
  function get_restaurants($page = 1){
    ...
    if ( false === $restaurants ) {
      $restaurants = array();
      $integrations_url = $this->zupplerhost . "/channels/" . $this->channel_slug . "/integrations.json?page=" . $page;

      $integrations = $this->get_resource($integrations_url, '_integrations_'.$page);
      if (!$integrations) { return false; }

      foreach($integrations["integrations"] as $i) {
        $restaurant = $this->get_resource($i["restaurant"]["resource_url"], '_restaurant_'.$i["restaurant"]["id"]);
        if(isset($restaurant)) {
          $restaurant["restaurant"]["permalink"] = $i["permalink"];
          $restaurant["restaurant"]["integration_permalink"] = $i["permalink"];
          array_push($restaurants, $restaurant["restaurant"]);
        }
      }
      if( count($restaurants) > 0 ) {
        set_transient( $this->channel_slug . '_restaurants_' . $page , json_encode($restaurants), $this->expiration );
      }
      return $restaurants;
    }
